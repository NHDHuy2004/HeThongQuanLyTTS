...
export function ReviewRequestSheet({
  ...
  const typeLabel = request?.type === 'wfh' ? 'Xin WFH' : 'Xin nghi phep'
  ...
  async function handleReview(decision: 'approved' | 'rejected') {
    ...
    setPending(false)
    if (res.success) {
      toast(decision === 'approved' ? 'Da duyet don thanh cong!' : 'Da tu choi don.')
      router.refresh()
      onClose()
    } else {
      toast(res.error ?? 'Khong the xu ly don.', 'error')
    }
  }

  return (
    <BottomSheet open={request !== null} onClose={onClose} title="Duyet don xin nghi">
      {request && (
        <div className="p-4">
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between gap-2">
              <div className="min-w-0">
                ...
                <p className="text-xs text-muted-foreground">
                  Gui {formatRelativeTime(request.created_at)}
                </p>
              </div>
            ...
            <div className="flex items-center gap-3 rounded-lg border border-border bg-muted/50 p-3">
              ...
              <p className="text-sm font-medium tabular-nums">
                {formatDate(request.start_date)} den {formatDate(request.end_date)}
              </p>
            </div>

            {/* Reason */}
            <div>
              <p className="mb-1 text-xs font-semibold text-muted-foreground">Ly do</p>
              <p className="whitespace-pre-wrap rounded-lg border border-border bg-card p-3 text-sm leading-relaxed">
                {request.reason || 'Khong co ly do.'}
              </p>
            </div>
          </div>
          ...
          <div className="mt-5 flex flex-col gap-2">
            ...
            >
              ...
              {pending ? 'Dang xu ly...' : confirmReject ? 'Xac nhan tu choi?' : 'Tu choi'}
            </Button>
          </div>
        </div>
      )}
