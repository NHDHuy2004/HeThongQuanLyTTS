...
export function ReviewTaskSheet({
  ...
  async function handleReview(decision: 'approved' | 'rejected') {
    ...
    setPending(false)
    if (res.success) {
      toast(decision === 'approved' ? 'Đã duyệt bài nộp!' : 'Đã trả bài kèm nhận xét.')
      router.refresh()
      onClose()
    } else {
  ...
  return (
    <BottomSheet open={task !== null} onClose={onClose} title="Duyệt bài nộp">
      {task && (
        <div className="p-4">
          {/* Title + status */}
          ...
          <div className="mt-4 flex items-center gap-3 rounded-lg border border-border bg-muted/50 p-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-xs text-muted-foreground">
                Nộp {task.submitted_at ? formatRelativeTime(task.submitted_at) : ''}
              </p>
            </div>
          </div>
          ...
          <div className="mt-4 space-y-2">
            {task.deadline && (
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="text-muted-foreground">Hạn chốt:</span>
          ...
          {task.description && (
            <div className="mt-4">
              <p className="mb-1 flex items-center gap-1.5 text-xs font-semibold text-muted-foreground">
                <Tag className="size-3.5" weight="bold" /> Mô tả
              </p>
          ...
          <div className="mt-4">
            <div className="mb-1 flex items-center gap-1.5">
              ...
              <p className="text-xs font-semibold text-muted-foreground">Nhận xét của bạn</p>
              <span className="ml-auto text-[10px] text-muted-foreground tabular-nums">{feedback.length}/2000</span>
            </div>
            <Textarea
              placeholder="Góp ý cho bài nộp này..."
              value={feedback}
            ...
            <p className="mt-1 text-[11px] text-muted-foreground">
              Bắt buộc nhập nhận xét khi từ chối bài.
            </p>
          </div>
          ...
          <div className="mt-5 flex flex-col gap-2">
            ...
            >
              ...
              {pending ? 'Đang xử lý...' : confirmReject ? 'Xác nhận trả bài?' : 'Trả bài kèm nhận xét'}
            </Button>
          </div>
        </div>
      )}
