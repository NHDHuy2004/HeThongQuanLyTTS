...
export function ReviewReportSheet({
  ...
  async function handleSubmit() {
    ...
    setPending(false)
    if (res.success) {
      toast('Đã phản hồi báo cáo!')
      router.refresh()
      onClose()
    } else {
      toast(res.error ?? 'Không thể gửi phản hồi.', 'error')
    }
  }

  return (
    <BottomSheet open={report !== null} onClose={onClose} title="Phản hồi báo cáo định kỳ">
      {report && (
        <div className="p-4">
          {/* Header */}
            ...
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-muted-foreground">
                  Kỳ {report.period_number ?? 1}
                </p>
              </div>
            </div>
            <Badge variant="info" className="shrink-0 text-[10px]">Đã nộp</Badge>
          </div>
          ...
          <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
            ...
            {report.submitted_at && (
              <span>Nộp {formatRelativeTime(report.submitted_at)}</span>
            )}
          </div>
          ...
          <div className="mt-4">
            <div className="mb-1 flex items-center justify-between">
              <p className="text-xs font-semibold text-muted-foreground">Phản hồi của bạn</p>
