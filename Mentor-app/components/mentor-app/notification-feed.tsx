...
export function NotificationFeed({
  ...
  const items = useMemo<FeedItem[]>(() => {
    const merged: FeedItem[] = [
      ...requests.map((r) => ({
        id: r.id,
        title: r.type === 'leave' ? 'Xin nghỉ phép' : 'Xin WFH',
        subtitle: `${r.profiles?.full_name ?? 'N/A'} - ${r.start_date} đến ${r.end_date}`,
        status: r.status,
        time: r.created_at,
      ...
      ...reports.map((r) => ({
        id: r.id,
        title: `Báo cáo định kỳ${r.period_number ? ` (Kỳ ${r.period_number})` : ''}`,
        subtitle: `${r.profiles?.full_name ?? 'N/A'} - Cần phản hồi`,
        status: r.status,
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  ...
                  {notif.type === 'request' && notif.status === 'pending' && (
                    <Badge variant="warning" className="shrink-0 text-[10px]">Mới</Badge>
                  )}
                  {notif.type === 'request' && notif.status !== 'pending' && (
