...
export async function checkIn() {
  ...
  if (profile && profile.role !== 'intern' && profile.role !== 'admin') {
    throw new Error('Chức năng điểm danh chỉ áp dụng cho Thực tập sinh.')
  }

  const { error } = await supabase.from('attendance').upsert(
    { intern_id: user.id, date: new Date().toISOString().slice(0, 10), check_in_time: new Date().toISOString(), status: 'present' },
