...
export default async function MentorRequestsPage() {
  ...
  // Fetch only requests for this mentor
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <div>
        ...
        <h1 className="text-2xl font-semibold tracking-tight">Duyệt đơn nghỉ phép & WFH của TTS</h1>
        <p className="mt-1 text-sm text-slate-500">
          Xem xét và phê duyệt các đơn xin nghỉ phép hoặc xin làm việc tại nhà từ sinh viên bạn hướng dẫn.
        </p>
      </div>
      ...
      {pendingRequests.length > 0 && (
        <section className="space-y-3">
          <h2 className="text-sm font-semibold text-orange-950 dark:text-orange-300 flex items-center gap-2">
            ...
            Đơn của TTS cần phê duyệt ({pendingRequests.length})
          </h2>
      ...
      <section className="space-y-3">
        ...
        {resolvedRequests.length ? (
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-8 text-center text-sm text-slate-400 dark:border-slate-800 dark:bg-slate-900">
            Chưa có đơn nào đã xử lý trong nhóm của bạn.
          </div>
        )}
      </section>
    </div>
